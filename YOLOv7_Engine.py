""" 
This script contains a class for YOLOv7 object detection and pose estimation, 
allowing the detection of objects in images and estimation of human pose. 
"""
...
from utils.general import check_img_size, non_max_suppression, scale_coords,  set_logging,  non_max_suppression_kpt
...
class YOLOv7():
    """
    Class for YOLOv7 object detection.
    """
    def __init__(self, weights: str = 'yolov7.pt', device: str = '', imgsz: int = 1280, conf_thres: float = 0.05, iou_thres: float = 0.65, *args, **kwargs):
        """
        Initializes the YOLOv7 object detector.

        Args:
            weights: Path to the YOLOv7 weights file.   
                    "yolov7-w6-pose.pt" for pose estimation
                    "yolov7.pt" for pose estimation
            device: Device to use for inference (default: CPU).
            imgsz: Size of the input image (default: 1280).
            conf_thres: Confidence threshold for object detection (default: 0.05).
            iou_thres: IOU threshold for non-maximum suppression (default: 0.65).
        """
        ...
        # Select the device for inference (CPU or GPU)
        self.device = select_device(device)
        # Enable half-precision (FP16) inference if running on a GPU
        ...
        # Determine the model stride and set it to an integer value
        ...
        self.imgsz = check_img_size(
            int(imgsz), s=self.stride)  # check img_size
    ...
    def detect(self, frame):
        ...
        if "pose" in self.weights:   # pose model
            output = non_max_suppression_kpt(pred, self.conf_thres, self.iou_thres,
        ...
        for i, det in enumerate(pred):  # detections per image
            if len(det):  # only when there are detections
                im0 = frame.copy()
